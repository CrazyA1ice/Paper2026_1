...
replacements = [
    ...
    (
        "历史窗口记为Xn",
        "第n个监督学习样本由历史窗口Xn和预测目标Yn组成。Xn包含连续L个时间点，Yn包含其后连续H个时间点。模型据此利用多个OD变量在过去L步中的联合变化，估计未来H步的取值。第n个样本对应的输入与目标定义为"
    ),
    (
        "本文取输入长度L=96",
    ...
    (
        "表3将核心模型扩展到随机种子42～49",
        "核心消融在随机种子42～49上重复8次。与单尺度DLinear相比，本文方法在Abilene和GÉANT上的平均MSE分别降低2.77%和2.37%。固定等权多尺度模型保留相同的尺度集合和3个DLinear专家，只把融合方式改为固定等权；与该模型相比，本文方法的平均MSE仍分别降低1.67%和4.68%。前一组比较反映完整多尺度模型相对单尺度基线的变化，后一组比较则把关注点进一步缩小到尺度融合方式。"
    ),
    (
