...
replacements = [
    ...
    (
        "两个数据集上的相对降幅存在差异",
        "两个数据集上的相对降幅并不相同。GÉANT包含的OD变量数高于Abilene，同时本文方法相对LightTS的MSE降幅也更大，但当前实验只能说明两种模型在这两个数据集上的差距不同，尚不能把这种差异直接解释为变量规模造成的结果。固定等权消融与本文方法采用相同的多尺度专家结构，因此更适合继续判断样本级融合方式本身带来的影响。"
    ),
    (
        "表3将核心模型扩展到随机种子42～49",
