...
replacements = [
    ...
    (
        ...
        "图6汇总随机种子42～49下全部测试窗口的尺度权重，用于检查路由网络是否形成不同于固定1/3的尺度分配。固定等权模型对所有样本使用同一组权重，本文方法则由每个输入窗口的统计特征计算权重，因此测试集上的分布可以直接显示路由器对三个尺度的总体偏好和变化范围。"
    ),
    (
        "Abilene中α1、α2和α4的平均权重依次为0.257",
        "Abilene中α1、α2和α4的平均权重依次为0.257、0.319和0.424，GÉANT中依次为0.283、0.318和0.399。两个数据集均表现为尺度4的平均权重最高、尺度1最低，且三个尺度的均值与固定等权的1/3存在差异。对于本文设置的24步预测任务，路由器整体上更偏向经过较强时间平滑的尺度4，但尺度1和尺度2仍保持非零权重，最终预测仍由不同时间分辨率共同构成。"
    ),
    (
