...
replacements = [
    ...
    (
        ...
        "表3将核心模型扩展到随机种子42～49，共8次重复实验。与单尺度DLinear相比，本文方法在Abilene和GÉANT上的平均MSE分别降低2.77%和2.37%。固定等权多尺度模型与本文方法使用相同的尺度集合和3个DLinear专家，只把融合权重固定为1/3；在这一对照下，本文方法的平均MSE仍分别降低1.67%和4.68%。这两组比较分别对应引入多尺度信息和改变尺度融合方式带来的结果变化。"
    ),
    (
    ...
    (
        ...
        "固定等权多尺度模型与本文方法共享尺度集合和三个DLinear专家，两者的主要区别是融合权重是否随样本变化。表3给出三类核心模型的绝对误差均值和标准差，图5补充8个共同随机种子的相对改进分布，并将比较扩展到MSE、MAE和RMSE。相对固定等权模型时，两个数据集三项指标的平均改进率均为正，说明误差变化不能只由增加多尺度专家解释，融合权重的设定也会影响结果。第2.5节据此继续检查路由器是否真正形成不同于1/3的尺度分配。"
    ),
    (
        "图6统计随机种子42—49下全部测试窗口的尺度权重",
        "图6汇总随机种子42～49下全部测试窗口的尺度权重。固定等权模型对所有样本使用同一组1/3权重，本文方法则由每个输入窗口的统计特征计算权重，因此测试集上的分布可以直接显示路由器对三个尺度的总体偏好和变化范围。"
    ),
    (
